 ...
 *
 */
public class ExpThreeRunnerMultipleDatasets {
		...
		try {
			...
			for (File dataFile : new File(dataDir).listFiles()){
				...
				if (dataFile.getName().endsWith(".txt")){
					...
					expanWriter.flush();
					ExpThreeRunner exp = new ExpThreeRunner(configurationFile.getAbsolutePath(), dataFile.getAbsolutePath());
					...
					writer_ignore.write(exp.printAllResults(1));
					writer_ignore.write(exp.printResultsInTable(1));
					writer_ignore.write(exp.printRecallPrecisionCurvesData(1));	

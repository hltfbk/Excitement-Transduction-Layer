 ...
 *
 */
public class TextCollectionWithBigramsAndFilteringByPos extends AbstractTextCollection  {
...
*/
	...
	protected final String m_posSeparator= "::";
	...
	public TextCollectionWithBigramsAndFilteringByPos(String configurationFileName) throws LemmatizerException, MalformedURLException, ConfigurationFileDuplicateKeyException, ConfigurationException{
		super();
	...
	public TextCollectionWithBigramsAndFilteringByPos(String configurationFileName, String dataFilename) throws LemmatizerException, MalformedURLException, ConfigurationFileDuplicateKeyException, ConfigurationException{
		super();
		setConfigurationFileName(configurationFileName);
...
*/	
				...
				if (pos.isEmpty()){ 
					...
					for (String expansionTerm : expansions.keySet()){ // for each expansion
						if (isInDomainVocabulary(expansionTerm)){ // don't add expansions which are not in the domain vocabulary (if given)
							...
							addExpansionTerm(expansionTerm, getExpansionWeight(confidence), termToExpand);
						}
						else{
					...
					for (String posString : pos){
						...
						for (String expansionTerm : expansions.keySet()){ // for each expansion
							if (isInDomainVocabulary(expansionTerm)){ // don't add expansions which are not in the domain vocabulary (if given)
								Double confidence = expansions.get(expansionTerm);
								addExpansionTerm(expansionTerm, getExpansionWeight(confidence), term);
							}
							else{
								System.out.println("!!!!!!!!! Discarded "+expansionTerm+" as an out-of-domain term.");
							}
	...
	private Double getExpansionWeight(Double confidence) {
		if (confidence > WordNetLexicalExpander.WN_CONFIDENCE) return 0.5; // if supported by more than one resource
		return 0.25; // if supported by a single resource		
	}

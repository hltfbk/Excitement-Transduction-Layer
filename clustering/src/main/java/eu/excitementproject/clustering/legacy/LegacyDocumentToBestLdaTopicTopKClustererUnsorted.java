...
public class LegacyDocumentToBestLdaTopicTopKClustererUnsorted extends DocumentToBestLdaTopicClusterer {
	
	public LegacyDocumentToBestLdaTopicTopKClustererUnsorted(boolean useExpandedCollection, TextCollection textCollection, String conf, int numOfTopics, WeightType weightType) throws ClusteringException {
		super(useExpandedCollection, textCollection, conf, numOfTopics, weightType);
	}
	
		
	public LegacyDocumentToBestLdaTopicTopKClustererUnsorted(boolean useExpandedCollection,
	...
	public Map<String, List<Integer>> clusterDocumentsByExternallModel(TextCollection textCollection, String modelFilename) throws Exception {			
		...
		if (firstAssignmentResults.size() > m_topKclusters){
			...
			if (m_topKclusters!=res.size()) System.err.println("Cannot produce the requested K="+m_topKclusters+" text clusters. Created "+res.size()+" clusters.");
			return res;			
		}

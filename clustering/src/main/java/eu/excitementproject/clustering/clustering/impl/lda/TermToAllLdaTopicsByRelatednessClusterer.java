 ...
 *  Local LDA model is created for the pseudo-documents, and each term (pseudo-document) is assigned to all of its topics 
 *    
 ...
 *
 */
public class TermToAllLdaTopicsByRelatednessClusterer extends AbstractTermByRelatednessLdaClusterer {

	public TermToAllLdaTopicsByRelatednessClusterer(boolean useExpandedCollection,
	...
	public Map<String, List<String>> clusterPseudoDocuments(TextCollection textCollection) throws Exception {			
		...
		for (String term : pseudoDocs.keySet()){			
			Map<String, Double> v = getTopicDistributionOfTerm(pseudoDocs.get(term));

...
public class FragmentAndModifierMatchCounter {
	 ...
	 * @return
	 */
	public static List<Integer> countFragmentCountsRelativeToBaseStatements(JCas sysOut, JCas gold) throws LAPException 
	{
		...
		while(goldTokenIter.hasNext())
		{
			...
			// only compare the tokens if it is not a modifier from the GoldStandard side (to take into account only the base statement
			Collection<ModifierAnnotation> goldMod = JCasUtil.selectCovering(gold, ModifierAnnotation.class, goldToken.getBegin(), goldToken.getEnd());
			if (goldMod == null || goldMod.size() == 0) {
	...
	public static String getClassDetails(List<Integer> counts) {
		// counts structure: [tp, fp, tn, fn]
		int posClass = counts.get(0) + counts.get(3);
		int negClass = counts.get(1) + counts.get(2);
		int all = posClass + negClass;
		
		return "Positive: " + posClass + " // Negative: " + negClass + " // Total: " + all;
	}
